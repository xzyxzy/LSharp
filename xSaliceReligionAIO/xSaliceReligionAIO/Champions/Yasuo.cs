...
namespace xSaliceReligionAIO.Champions
{
    class Yasuo : Champion
    {
        ...
        {
            int hit = 0;
            foreach (var target in ObjectManager.Get<Obj_AI_Hero>().Where(x => Player.IsValidTarget(R.Range) && (x.HasBuffOfType(BuffType.Knockup) || x.HasBuffOfType(BuffType.Knockback))))
            {
                hit = 1;
